...
class activity_navigation implements renderable, templatable {
    ...
    public function __construct($prevmod, $nextmod, $activitylist = array()) {
        ...
        if ($prevmod) {
            ...
            $this->prevlink = new \action_link($linkurl, $OUTPUT->larrow() . ' ' . 'Previous Lesson', null, $attributes);//Eruditiontec
        }
        ...
        if ($nextmod) {
            ...
            $this->nextlink = new \action_link($linkurl, 'Next Lesson' . ' ' . $OUTPUT->rarrow(), null, $attributes);//Eruditiontec
        }
