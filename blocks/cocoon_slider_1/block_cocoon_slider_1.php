                ...
                if (!empty($data->$sliderimage)) {
                    ...
                    <div class="slide slide-one" style="background-image: url(' . moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/{$this->context->id}/block_cocoon_slider_1/slides/" . $i . '/' . $mainfile) . '); height: 95vh;width:100%;">
                                ...
                                if (!empty($data->$slide_btn_text)) {
					                        $text .='<div class="btn-block"><a target="'.$data->$slide_btn_target.'" href="'.format_text($data->$slide_btn_url, FORMAT_HTML, array('filter' => true)).'" class="banner-btn">'.format_text($data->$slide_btn_text, FORMAT_HTML, array('filter' => true)).'</a></div>';
                                }
                                $text .='
					                    </div>
					                </div>
